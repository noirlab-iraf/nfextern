...
begin
	...
	string	tmp1, tmp2, tmp3, ttmp1, ttmp2, tmpresults, buf, lrewrite
	...
	tcopy (ltable, ttmp2, verbose-)
	tcalc (ttmp2, "rowno", "rownum", datatype="int", colunits="",
	    colfmt="%5d")
	...
	tselect (ttmp2, ttmp1, buf)
	tdelete (ttmp2, ver-, >& "dev$null")
...
#	    geom = fitgeometry
	...
	tcalc (ttmp1, "RA_HRS", "RA_DEG / 15.0d0", datatype="double",
	    colunits="", colfmt="%13.4h")

	tchcol (ttmp1, "X_CENTER", "", "%10.3f", "", verbose=no)
	tchcol (ttmp1, "Y_CENTER", "", "%10.3f", "", verbose=no)
	tchcol (ttmp1, "DEC_DEG", "", "%13.3h", "", verbose=no)

	#outpixsys = inpixsys
	if (inpixsys != outpixsys) {
	    tprint (ttmp1, columns="x_center,y_center,ra_hrs,dec_deg",
	...
	if (tabupdate) {
	    ...
	    tdelete (ttmp1, ver-, >& "dev$null")

	    printf ("done\n")

	    if (refitcat) {
		tcopy (ltable, ttmp1, verbose-)
		tcalc (ttmp1, "rowno", "rownum", datatype="int", colunits="",
		    colfmt="%5d")

		buf =	catpars.cen_col // " == 0 && " //
			catpars.obj_col // " == 0"
		tselect (ttmp1, ttmp2, buf)
		tdelete (ttmp1, ver-, >& "dev$null")
		...
		if (tinfo.nrows >= 1) {
		    ...
		    tcalc (ttmp2, "RA_HRS", "RA_DEG / 15.0d0", datatype="double",
			colunits="", colfmt="%13.4h")

#		    tchcol (ttmp2, "X_CENTER", "", "%8.2f", "", verbose=no)
#		    tchcol (ttmp2, "Y_CENTER", "", "%8.2f", "", verbose=no)
		    tchcol (ttmp2, "DEC_DEG", "", "%13.3h", "", verbose=no)

		    tprint (ttmp2, columns="rowno,RA_HRS,DEC_DEG",
		...
		if (tinfo.nrows >= 1) {
		    ...
		    tchcol (ttmp2, "X_CENTER", "", "%10.3f", "", verbose=no)
		    tchcol (ttmp2, "Y_CENTER", "", "%10.3f", "", verbose=no)

		    tprint (ttmp2, columns="rowno,x_center,y_center",
			prparam-, prdata+, pwidth=80, plength=0, showrow-,
			showhdr-, rows="-", option="plain", sp_col="",
			lgroup=0, > tmp1)
