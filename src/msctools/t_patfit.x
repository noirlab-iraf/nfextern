...
begin
	...
	do i = 1, 2 {
	    ...
	    call fprintf (logfd[i], "  statwt = %.6g\n")
		call pargd ((stat[PW] - stat[QW] - stat[PV] + stat[QV]) *
		    (stat[N] / 1000000))
...
begin
	...
	do i = 1-nl1, nl-nl2 {
	    ...
	    # This is inefficient but we are (maybe) working with
	    # the buffer in mapio.
	    if (bkinbuf != NULL && nblk > 1)
		call adivkr (Memr[bkinbuf], real(nblk), Memr[bkinbuf], nc)
	    if (bkpatbuf != NULL && nblk > 1)
		call adivkr (Memr[bkpatbuf], real(nblk), Memr[bkpatbuf], nc)
	    if (bkwtbuf != NULL && nblk > 1)
		call adivkr (Memr[bkwtbuf], real(nblk), Memr[bkwtbuf], nc)
	}
	sum[N] = n
	n = n / nblk
