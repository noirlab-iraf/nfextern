...
call eprintf ("ic_fvshow: B\n")
	    call rcvvector (cv, x, Memr[fit], n)
call eprintf ("ic_fvshow: C\n")
	    call rcvcoeff (cv, Memr[coeffs], ncoeffs)
call eprintf ("ic_fvshow: ncoeffs = %d\n")
call pargi (ncoeffs)
call eprintf ("ic_fvshow: D\n")
	    call rcverrors (cv, y, Memr[wts1], Memr[fit], n, chisqr,
		Memr[errors])
call eprintf ("ic_fvshow: E\n")
	    rms = ic_rmsr (x, y, Memr[fit], Memr[wts1], n)
call eprintf ("ic_fvshow: F\n")
	} else {
	    # Allocate memory for the fit.
