...
begin
	...
	im[1] = in; im[2] = NULL
	do l = 1, nl, blkstep {
	    call convolve (im, bpm, skymap, sigmap, expmap, bpval, bpdet, bpflg,
...
begin
	...
	call calloc (SKB_NSKYUT(skb), SKB_NCSBLK(skb), TY_INT)
	call calloc (SKB_SKYSUT(skb), SKB_NCSBLK(skb)*SKB_NLSBLK(skb), TY_REAL)
	call calloc (SKB_SIGSUT(skb), SKB_NCSBLK(skb)*SKB_NLSBLK(skb), TY_REAL)
	...
	SKB_SKYUT(skb) = SKB_SKYSUT(skb)
	SKB_SIGUT(skb) = SKB_SIGSUT(skb)
...
begin
	call mfree (SKB_NSKYUT(skb), TY_INT)
	call mfree (SKB_SKYSUT(skb), TY_REAL)
	call mfree (SKB_SIGSUT(skb), TY_REAL)
...
real	a, b, s, t, x, rcnv, tcnv
int	c, n, ncmax, nbins, bin, csky
pointer	bins, skysut, sigsut, nskyut, skys, sigs, exps, nsky
...
begin
	...
	nbins = SKB_NBINS(skb)
	bins = SKB_BINS(skb)
	skysut = SKB_SKYUT(skb)
	sigsut = SKB_SIGUT(skb)
	nskyut = SKB_NSKYUT(skb)
	skys = SKB_SKY(skb)
	sigs = SKB_SIG(skb)
	exps = SKB_EXP(skb)
	nsky = SKB_NSKY(skb)
	...
	if (IS_INDEFR(Memr[exps])) {
		...
		x = a * rcnv / tcnv + b
		csky = (c-1) / n
		Memr[skysut+csky] = Memr[skysut+csky] + s
		Memr[sigsut+csky] = Memr[sigsut+csky] + t
		Memi[nskyut+csky] = Memi[nskyut+csky] + 1

		if (bp[c] != 0 || x < 1 || x > nbins)
		    next

		bin = bins + csky * nbins + x - 1
		Memi[bin] = Memi[bin] + 1
	...
	if (n == 0 || n + blkstep > SKB_NLSPIX(skb)) {
	    n = SKB_NCSBLK(skb)
	    call skb_blkeval (Memi[bins], nbins, a, b,
	        Memr[skysut], Memr[sigsut], Memi[nskyut], Memr[skys],
		Memr[sigs], Memr[exps], Memi[nsky], n, SKB_NSKYMIN(skb),
		SKB_NAV(skb), SKB_HISTWT(skb), SKB_SIGFAC(skb))

	    # Initialize for accumulation of next line of blocks.
	    SKB_SKYUT(skb) = skysut + n
	    SKB_SIGUT(skb) = sigsut + n
	    SKB_SKY(skb) = skys + n
	    SKB_SIG(skb) = sigs + n
	    if (!IS_INDEFR(Memr[exps]))
		call aclrr (Memr[exps], n) 
	    call aclri (Memi[nskyut], n) 
...
procedure skb_blkeval (bins, nbins, a, b, skysut, sigsut, nskyut,
	skys, sigs, exps, nsky, ncsblk, nskymin, nav, histwt, sigfac)

int	bins[nbins,ncsblk]	#I Sky subblock bins
int	nbins			#I Number of bins
real	a, b			#I Binning coefficients
real	skysut[ncsblk]		#U Unthresholded sky sum in /avg out
real	sigsut[ncsblk]		#U Unthresholded sigma sum in / avg out
int	nskyut[ncsblk]		#I Number of unthresholded values in bin
...
begin
	...
	if (dosky) {
	    ...
	    call skb_merge (Memr[SKB_SKYS(skb)], Memr[SKB_SKYSUT(skb)],
	        SKB_NCSBLK(skb), SKB_NLSBLK(skb), Memr[SKB_SKYS(skb)],
		SKB_NCBLK(skb), SKB_NLBLK(skb))
	...
	if (dosig) {
	    ...
	    call skb_merge (Memr[SKB_SIGS(skb)], Memr[SKB_SIGSUT(skb)],
	        SKB_NCSBLK(skb), SKB_NLSBLK(skb), Memr[SKB_SIGS(skb)],
		SKB_NCBLK(skb), SKB_NLBLK(skb))
...
procedure skb_merge (in, in1, ncin, nlin, out, ncout, nlout)

real	in[ncin,nlin], in1[ncin,nlin]
int	ncin, nlin
...
begin
	...
	if (nindef > 0) {
	    ...
	    iferr (call interp2 (out, Memr[work], ncout, nlout)) {
		#call erract (EA_WARN)
		call erract (EA_ERROR)
	        call amovr (in1, out, ncout*nlout)
	    } else
...
## Use order selection.
#
#procedure skb_merge (in, in1, ncin, nlin, out, ncout, nlout, select)
#
#real	in[ncin,nlin], in1[ncin,nlin]
#int	ncin, nlin
#real	out[ncout,nlout]
