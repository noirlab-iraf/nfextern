...
begin
	...
	do l = 1, nl {
	    # Get data.
	    call convolve (im, bpm, skymap, sigmap, expmap,
...
int	n, ncmax, nlmax, nbins, csky, ori()
...
begin
	...
	do c = 1, nc {
		...
		if (hdetect) {
			...
			z, s, t, z2, s2, t2, neighbors, bp[c], "------", num)
			...
			if (rcnv > 1.)
			    rcnv = (4 * rcnv**0.25 - 3)
			bin = min (nint (rcnv * binscale), 10000)
		...
		if (ldetect) {
			...
			Memi[ids], Memi[links], Memi[objs], nobjs, nalloc,
			2*s-z, s, t, z1, s1, t1, neighbors, bp[c], "-D---", num)
...
# DADD -- Add a pixel to the object list and set the mask value.

procedure dadd (c, l, z, zlast, nc, ids, links, objs, nobjs, nalloc,
	data, sky, sigma, data2, sky2, sigma2, neighbors, bp, flags, num)
...
begin
	...
	if (num == INDEFI) {
	    ...
	    OBJ_ISIGAVG(obj) = 0.
	    OBJ_ISIGAV(obj) = 0.
	    call strcpy (flags, OBJ_FLAGS(obj), SZ_FLAGS)
	}
	obj = objs[num]
	...
	if (merge) {
		...
		obj1 = objs[num1]
		OBJ_SIG(obj) = OBJ_SIG(obj) + OBJ_SIG(obj1)
		...
		if (OBJ_ISIGMAX(obj) < OBJ_ISIGMAX(obj1)) {
		    ...
		    OBJ_XPEAK(obj) = OBJ_XPEAK(obj1)
		    OBJ_YPEAK(obj) = OBJ_YPEAK(obj1)
		}
...
begin
	...
	do i = NUMSTART, nobjs {
	    ...
	    OBJ_PEAK(obj) = OBJ_ISIGMAX(obj) * OBJ_SIG(obj)
...
begin
	...
	do l = 1, nl {
		...
		if (id >= NUMSTART) {
		    ...
		    if (id > 0) {
			if (MBP(rl[3,i]))
			    id = MSETFLAG(id,MASK_BP)
			if (OBJ_FLAG(obj,BP)=='B')
			    id = MSETFLAG(id,MASK_BPFLAG)
...
begin
	...
	#call strcpy ("------", OBJ_FLAGS(obj), SZ_FLAGS)
	OBJ_FLUX(obj) = INDEFR
	OBJ_SKY(obj) = INDEFR
	OBJ_PEAK(obj) = INDEFR
