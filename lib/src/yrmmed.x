...
real procedure yrm_med (rm, nclip, navg, blank, exclude, index, in, mask, nused)
...
begin
	...
	rms = RM_RMS(rm)
	val = yrmsorted (rms, nclip, index, in)
...
real procedure yrm_gmed (rm, nclip, navg, blank, exclude, nused)
...
real procedure yrm_gdata (rm, index)
...
pointer procedure yrm_open (box, type, ndatasets, pixtype)
...
begin
	...
	case RM_TYMED:
	    val = 0.
	    rms = yrms_open (box, RMS_TYMED, val)
	case RM_TYMAX:
	    switch (i) {
	    case TY_SHORT:
		val = -MAX_SHORT
		rms = yrms_open (box, RMS_TYMAX, val)
	    case TY_REAL:
		val = -MAX_REAL
		rms = yrms_open (box, RMS_TYMAX, val)
	    }
	case RM_TYMIN:
	    ...
	    case TY_SHORT:
		val = MAX_SHORT
		rms = yrms_open (box, RMS_TYMIN, val)
	    case TY_REAL:
		val = MAX_REAL
		rms = yrms_open (box, RMS_TYMIN, val)
	    }
...
procedure yrm_close (rm)

pointer	rm			#I RM pointer

begin
	call yrms_close (RM_RMS(rm))
...
begin
	...
	if (RM_PIXTYPE(rm) == TY_SHORT)
	    call yanirs (DATA(rms,0), PWINS(rm,dataset), RM_BOX(rm))
#	else
...
procedure yrm_unpack (rm, dataset)
...
procedure yrm_dump (rm, unsorted, sorted, in, out)
...
begin
	call yrms_dump (RM_RMS(rm), unsorted, sorted, in, out)
end
